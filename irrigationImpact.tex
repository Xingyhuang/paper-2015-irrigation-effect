...
In this paper, we applied the recently developed variable-resolution option within the Community Earth System Model (VR-CESM) to study the irrigation effect on regional climate over Central Valley in California. The irrigation practice plays important role in regulating the climate patterns of heavily irrigated regions. A flexible irrigation scheme with relatively realistic estimates of regional agricultural water use is enabled for the land model coupled in the CESM. We investigated the impact of irrigation not only on mean climatology, but also on heat extremes over from year 1980-01-01 to 2005-12-31 with the fine grid resolution at $0.25^\circ$ ($\sim$28 km). Simulation results from a control run without irrigation and two irrigation-enabled runs are analyzed, comparing to gridded observations and weather station datasets. During the summer (when irrigation peaks), cooling effect caused by irrigation is obvious for daily maximum near-surface temperature (Tmax) with the seasonally averaged magnitude around 1.3 K, in constrast to California’s dry Mediterranean climate. The latent heat flux is increased by 71$\%$ during the daytime caused by evaporation from the surface after adding ?1.27 mm/day irrigated water. The specific humidity also increased by about 15$\%$, and the averaged soil moisture show a significant increase though with a slight amplitude (?5$\%$) under irrigation. The frequency distribution of Tmax is improved, and both length and duration of hot spells are well captured with irrigation enabled. For cropland, the heat stress frequency has been reduced for about 34$\%$ under irrigation. 
...
As a state-of-the-art Earth modeling framework, CESM 1.2.0 consists of coupled atmospheric, oceanic, land and sea ice models \citep{neale2010description, hurrell2013community}. In this study, CAM version 5 (CAM5) \citep{CAM5Tech} and the Community Land Model (CLM) version 4.0 \citep{CLM40Tech} are used. The finest horizontal resolution of our grid is 0.25 $^\circ$ ($\sim$ 28km) covering the western U.S., with a quasi-uniform 1 $^\circ$ mesh over the remainder of the globe (See Figure \ref{fig:Figure 1}). Considering the relatively flat topography (less than 100m) over most of CV, the $\sim$ 28km grid resolution satisfies our need for modeling irrigation effects. Detailed descriptions of VR-CESM employed in this paper can be found in \cite{rhoades2015characterizing}, here, we only focus on the irrigation scheme within CLM 4.0.

The irrigation model we use is described in \cite{sacks2009effects}.  The fractional area of the cropland within each grid cell (independent of specific type) that is equipped for irrigation is from \cite{siebert2005development, siebert2007global} for the year 2000, and is fixed over the simulation period (see Figure \ref{fig:Figure 2}). This assumption is reasonable since irrigated area has been largely unchanged in California since 1980 \citep{bonfils2007empirical}. 
...
Simulation results were not detrended since the linear trend was small ($\sim$0.06 $^\circ$C/year){\color{red}[Huang: Is this ok for this study?]}. To restrict the analysis to the CV, the variables of interest have been masked and/or averaged within the area defined by the bounded region depicted in Figure \ref{fig:Figure 2}, which contains 155 degrees of freedom.

To quantify the model performance in comparison with the reference datasets, the root-mean-square deviation (RMSD) and mean signed difference (MSD) are used, and spatial correlation (Corr) is assessed by computing Pearson product-moment coefficient of linear correlation. Mathematically, these quantities are written as,
\begin{align}
RMSD &= \sqrt{\frac{1}{N} \sum_{i=1}^{N} (v_i - \hat{v}_i)^2}  & MSD &= \frac{1}{N} \sum_{i=1}^{N} (v_i - \hat{v}_i) 
\end{align} where $v_i$ and $\hat{v}_i$ are values from the simulation output and reference dataset, respectively; $i$ is the grid-point index and N is the total number of grid points over specific regions. {\color{red}not sure to add this}

{\color{red} Huang: add info for supplement: Complementary results to this study are provided in the online supplement, including the xxx.}
...
In order to understand the processes by which the irrigation model effects surface cooling, we have tabulated key variables in Table \ref{tab:table2}. Box and whisker plots showing the interannual variability are given in Figure \ref{fig:Figure 4}. The scarcity of natural precipitation ($\sim$0.1 mm/day) and summertime heating has led to a 456\% increase {\color{red}[Check. Huang: That's exact what the output gives. The total latent heat flux is including the vegetation part. In this way, the value matches.]} in ground evaporation and a 71\% increase in latent heat flux after adding $\sim$1.27 mm/day irrigated water for IRG(0.5) in such a dry and hot summer period. The near-surface mean and maximum temperature is cooled due to the largely increased latent heat flux during the daytime caused by evaporation from the surface. With irrigation enabled, the specific humidity increased by about 15$\%$ due to increased evaporation and the sensible heat flux decreased by 16$\%$ with lower surface temperature and shifting of sensible to latent heat flux. Nonetheless, the average soil moisture over all the subsurface layers has only shown a slight increase ($\sim5\%$) under irrigation, although the value is statistically significant under the t-test ($p < 0.05$) {\color{red}[Check my p value. Huang: KS test p-value is 2.103e-13]}.  This result suggests that irrigated water does not effectively infiltrate into lower soil layers. Although the irrigated water added in IRG(0.5) was nearly half of that of IRG, it did not lead to significant differences among relevant variables, and simply lead to larger runoff (remaining from total surface water after infiltration into the soil) in IRG ($\sim$1.6 mm/day) compared with IRG(0.5) ($\sim$0.24 mm/day).
...
Based on the seasonal averaged values over 26 years, the distributions of selected variables are displayed by the box and whisker diagram in Figure \ref{fig:Figure 4}. With irrigation, both the average magnitude {\color{red}[is median ok?]} and annual variability of Tmax becomes notably closer to observations. Comparing to NRG, the range of Tmax for irrigation runs reduced to $\sim$3$^\circ$C from $\sim$4.5$^\circ$C with less concentrated distribution. Though average precipitation did not differ among these three runs, adding irrigation tends to widen the range of precipitation intensity and bring more extreme precipitation events {\color{red} [possible reasons]}. NRG captures the distribution of JJA Pr better than the IRG, with reduced variability, though the reference datasets (i.e. UW and PRISM) also differ a little from each other. The range of yearly irrigated water widened proportionally as the average amount increased. Though the mean latent heat flux almost doubles for irrigation runs, the main features of distribution did not really differ from NRG. The range and variability of soil moisture significantly decreased after adding more irrigated water, though the mean value only increased a little (~1$\%$) comparing the two irrigation runs.
...
Due to the important role CV plays in agricultural industry, we have also looked at the heat stress for crops. To do so, we calculated the hours that exceeded 35$^\circ$C (as used in \cite{teixeira2013global}) over each day of from June 1st to September 30th (JJAS). Then, we used cosine function to model hourly temperatures based on daily Tmax and Tmin, and compared the model output with the hourly dataset we have for year 2000. We found that the error of the cosine function fitting results (i.e. hours exceeding 35$^\circ$C) is only about 4$\%$. Therefore, this cosine function is further applied to get the hourly temperatures over 26 years in CV. Based on the averaged hourly outputs over CV area, we found that (shown in Figure \ref{fig:Figure 6}), both the heat stress intensity and frequency are reduced under irrigation. The average hours over all days during JJAS are 2.152 for NRG and 1.424 for IRG (about 34$\%$ less).
...
\begin{table}
\begin{center}
\caption{Key variables associated with irrigation in the CV over all 1980-2005 JJA seasons. {\color{red}[Why the asterisks? Huang: Variables with statistically significant difference between NRG and IRG are marked with asterisks]}} \label{tab:table2}
\begin{tabular*}{7.5in}{l @{\extracolsep{\fill}}cccccccccccc}
...
{\color{red} change the order of variables, remove the 10m wind and add QVEGE and QVEGT, change the unit of Soil moisture to mm3/mm3?}

...
\title{Irrigation impacts on California's climate in the variable-resolution CESM}
%
...
With irrigation employed, nighttime warming is expected to occur, leading to an increase in daily Tmin due to the increased thermal conductivity of wet soil, as found by \cite{kanamaru2008model}, or with increased heat capacity of the soil and vegetation as argued by \cite{bonfils2007empirical}.  However, in our irrigation-enabled runs, Tmin did not change significantly, since soil moisture only increased by about 5$\%$. {\color{red}[As far as I know, observations show an increase in Tmin over the study period.  Do you think this is a deficiency in the irrigation model?]  Within the CV, CESM already has a known warm bias when compared with observations that may mitigate the warming due to irrigation (Huang et al., 2016).}
...
\begin{table}
\begin{center}
\caption{RMSD, MSD and Corr of Tmax between models and gridded observations over the CV in JJA (1980-2005).} \label{tab:table1}
...
\caption{The fraction of irrigated cropland at each grid cell (percent).  The blue line denotes the boundary of the CV region. {\color{red}[Change capitalization in title: ``Fraction of Irrigated Cropland''}}
\label{fig:Figure 2}
\end{center}
\end{figure}
...
\caption{Box plots for (a) Tmax, (b) Latent heat flux, (c) Precipitation, and (e) Soil moisture.  From top to bottom, horizontal lines represent maximum value, third quartile, median, first quartile and minimum value, respectively. {\color{red}[Put titles on top of plots]}} 
\label{fig:Figure 4}
\end{center}
\end{figure}
...
\caption{Frequency distribution of JJA daily T$_{max}$ over the simulation period 1981-2005 from simulations and reference datasets. {\color{red}[Correct:  Tmax and Frequency (\%) use different font sizes]}}
\label{fig:Figure 5}
\end{center}
\end{figure}
...
\caption{The number of hours that larger or equal to 35$^\circ$C per day from June 1st to September (Sept.) 30th averaged over 1980-2005, for NRG and IRG runs. {\color{red}[Replace $>=$ with $\geq$]}}
\label{fig:Figure 6}
\end{center}
\end{figure}
